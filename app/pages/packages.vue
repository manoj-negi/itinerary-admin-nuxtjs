  ...
  <div>
    ...
    <div :class="showForm ? 'hidden' : 'block'">
      <div class="flex justify-between items-center mb-12">
        <div>
          <h1 class="text-4xl font-bold text-gray-900 dark:text-slate-50 mb-2">
            Packages Management
          </h1>
        </div>
        <button
          class="px-8 py-3 bg-green-500 text-white rounded-xl hover:bg-green-600 font-semibold shadow-sm"
          @click="openCreate"
        >
          + New Package
        </button>
          ...
          >
            ...
            <tbody>
              <tr
                ...
                :key="p.id"
                class="hover:bg-gray-50 dark:hover:bg-slate-800 text-sm"
              >
                ...
                <td class="border-b border-gray-200 dark:border-slate-700 p-4">
                  <span :class="p.is_featured ? 'text-green-600' : 'text-gray-500'" class="text-sm font-medium">
          ...
          <form @submit.prevent="submitPackage" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="flex items-center">
                <label class="text-sm font-medium text-gray-700 dark:text-slate-300 flex items-center gap-2 mt-6">
            ...
            <!-- Images Section (same as Tours) -->
            <div>
              <label class="block text-sm font-medium mb-4 text-gray-700 dark:text-slate-300">
                Package Images {{ isEditing ? '(Optional - will replace existing)' : '' }}
              </label>
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              <button
                type="submit"
                :disabled="isSubmitting || !form.tour_id || !form.package_name || !form.price || !form.currency"
              ...
              >
                ...
                <span v-else>{{ isEditing ? 'Update Package' : 'Create Package' }}</span>
              </button>
            </div>
          </form>
    ...
    const res = await fetch('/api/graphql', {
      ...
      body: JSON.stringify({
        ...
        variables: isUpdate
          ? {
              id: form.value.id,
              tour_id: form.value.tour_id || undefined,
              package_name: form.value.package_name || undefined,
          ...
          : {
              ...
              currency: form.value.currency || 'INR',
              occupancy: form.value.occupancy || null,
              is_featured: form.value.is_featured,
              images: imagesInput
            }
      })
...
const loadPackages = async () => {
  try {
    ...
    alert(`Failed to load packages: ${err.message}`)
  }
}
